<x-app-layout>
    ...
    <div class="px-5 py-4 sm:px-6 sm:py-6 flex">
        <h3 class="text-base font-medium text-gray-800 dark:text-white/90">
            Ajouter un document pedagogique
        </h3>
    </div>

    <form action="{{ route('document-pedagogiques.store') }}" method="POST">
        @csrf
        <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="grid grid-cols-12 gap-6 border-t border-gray-100 p-5 sm:p-6 dark:border-gray-800">
                ...
                <div class="col-span-5">
                    ...
                    <x-text-input id="titre" name="titre" type="text" :value="old('titre')" />
                    <x-input-error :messages="$errors->get('titre')" class="mt-2" />
                </div>
                <div class="col-span-4">
                    <x-input-label for="auteur" value="Auteur" />
                    <x-text-input id="auteur" name="auteur" type="text" :value="old('auteur')" />
                    <x-input-error :messages="$errors->get('auteur')" class="mt-2" />
                </div>

                <div class="col-span-4">
                    <x-input-label for="quantité" value="Quantité" />
                    <input id="quantité" name="quantité" type="number" value="{{ old('quantité') }}"
                    ...
                    <x-input-error :messages="$errors->get('quantité')" class="mt-2" />
                </div>
